...
import math
from ReoMusic.utils.formatters import time_to_seconds
...
def stream_markup_timer(_, videoid, chat_id, played, dur):
    ...
    ReoMusic = math.floor(percentage)
    if 0 < ReoMusic <= 10:
        bar = "𖢵—————————"
    elif 10 < ReoMusic < 20:
        bar = "—𖢵————————"
...
def playlist_markup(_, videoid, user_id, ptype, channel, fplay):
    buttons = [
        [
            InlineKeyboardButton(
                text=_["P_B_1"],
                callback_data=f"ReoMusicPlaylists {videoid}|{user_id}|{ptype}|a|{channel}|{fplay}",
            ),
            InlineKeyboardButton(
                text=_["P_B_2"],
                callback_data=f"ReoMusicPlaylists {videoid}|{user_id}|{ptype}|v|{channel}|{fplay}",
            ),
        ],
        [
            InlineKeyboardButton(
